...
export function LettersLadderCard({
  caseId,
  letters,
  letterUnlocked,
  l1Sent,
}: LettersLadderCardProps) {
        ...
        {rows.map((row) => (
          <div
            ...
            className={cn(
              ...
              row.locked
                ? "border-dashed border-[var(--border)] opacity-65"
                : "border-[var(--border)]",
            )}
          >
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-px text-xs text-[var(--ink-faint)]">
                {row.note}
              </p>
            </div>
            {row.chip ? (
            ...
            {row.open ? (
              <Link
                ...
                className="flex min-h-[44px] flex-none items-center rounded-lg border border-[var(--border-strong)] bg-[var(--surface-raised)] px-3 text-[0.78125rem] font-semibold text-[var(--color-sky-deep)] no-underline"
              >
                {t("open")}
