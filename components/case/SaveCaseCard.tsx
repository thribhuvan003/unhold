...
export function SaveCaseCard({
  ...
  const smsRecoveryEnabled =
    process.env.NEXT_PUBLIC_ENABLE_SMS_RECOVERY === "true";
  const [copied, setCopied] = useState<"code" | "recovery" | "link" | null>(
    null,
  );
    ...
    return (
      ...
      >
        ...
        <button
          ...
          className="min-h-[44px] rounded-lg border border-[var(--border)] px-2 font-semibold text-[var(--color-sky-deep)]"
        >
  ...
  return (
    <section
      ...
      className={cn("u-card flex flex-col gap-3 p-4", className)}
    >
      <div>
        <p className="type-eyebrow">{t("eyebrow")}</p>
        <h1 className="type-display mt-2 text-xl text-[var(--ink)]">
          {t("title")}
        </h1>
      ...
      <div className="rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] px-3 py-3">
        ...
        <p className="type-mono-data mt-1 text-lg text-[var(--ink)]">
          {publicId}
        </p>
        <button
          type="button"
          onClick={() => copy("code", publicId)}
          className="u-btn u-btn-ghost mt-2 min-h-[44px] w-full text-sm font-semibold"
        >
      ...
      {recoveryCode ? (
        <div className="rounded-[var(--radius-md)] border border-[var(--saffron)]/40 bg-[var(--warn-muted)] px-3 py-3">
          ...
          <p className="type-mono-data mt-1 text-lg tracking-widest text-[var(--ink)]">
            {recoveryCode}
          </p>
          <p className="mt-1.5 text-xs leading-normal text-[var(--ink-muted)]">
            {t("recoveryHint")}
          </p>
          <button
            type="button"
          ...
          {smsRecoveryEnabled ? (
            <div className="mt-3 border-t border-[var(--saffron)]/25 pt-3">
              ...
              <p className="mt-1 text-[0.6875rem] leading-relaxed text-[var(--ink-muted)]">
                {t("smsHint")}
              </p>
              <div className="relative mt-2">
                ...
                <input
                  ...
                  value={smsPhone}
                  onChange={(e) =>
                    setSmsPhone(e.target.value.replace(/\D/g, "").slice(0, 10))
                  }
              ...
              <button
                ...
                className="u-btn u-btn-secondary mt-2 min-h-[44px] w-full text-sm font-semibold disabled:opacity-50"
              >
                {smsState === "sending"
                  ? t("smsSending")
                  : smsState === "sent"
      ...
      <div className="rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] px-3 py-3">
        ...
        <p className="mt-1 break-all text-xs text-[var(--color-sky-deep)]">
          {caseUrl(caseId)}
        </p>
        <button
          type="button"
