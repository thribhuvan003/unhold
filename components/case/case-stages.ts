...
export type CaseStageInput = {
  ...
  l1Drafted: boolean;
  l1Sent: boolean;
  /** Reason-aware unlock from isLetterEvidenceReady; overrides cyber default. */
...
export function computeCaseStages(input: CaseStageInput, locale = 'en'): CaseStageModel {
  const {
    ...
    l1Drafted,
    l1Sent,
    letterUnlocked: letterUnlockedOverride,
  } = input;
  const hi = locale === 'hi';
  ...
  // Prefer reason-aware unlock from the page; fallback matches cyber checklist.
  const letterUnlocked =
    letterUnlockedOverride ?? (hasFreezeNotice && hasBankStatement);
