...
export function MarkSentForm({
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (!proofFile) {
      setError(isFinal ? t('addProofFirstFinal') : t('addProofFirst', { next }));
      return;
    }
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-[0.78125rem] leading-normal text-[var(--ink-muted)]">
        {isFinal ? t('subtitleFinal') : t('subtitle', { next })}
      </p>
      ...
      {success ? (
        <p role="status" aria-live="polite" className="u-alert u-alert-success mt-2.5">
          {t('successNote')}
        </p>
      ) : null}
    </form>
