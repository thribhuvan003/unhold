...
interface PapersChecklistProps {
  ...
  /** When false, skip AI poll copy and swarm polling (consent opt-out). */
...
// ~2s cadence stays under the 180/5min swarm-events budget with headroom.
...
export function PapersChecklist({
  ...
  async function pollVerification(
    ...
    for (let attempt = 0; attempt < POLL_MAX_ATTEMPTS; attempt += 1) {
      ...
      try {
        ...
        if (res.status === 429) return "timeout";
        if (!res.ok) continue;
  ...
  async function handleUpload(type: PaperType, file: File, isCore: boolean) {
    ...
    try {
      ...
      if (!aiOcrEnabled) {
        // Consent opt-out: seal only — never claim AI is checking.
        patchRow(type, { status: "done", pending: false, verification: null });
      } else {
        void (async () => {
          if (isCore) {
            const verification = await pollVerification(evidenceId);
            // Always clear "checking" — timeout means background job continues,
            // not that the UI should spin forever.
            patchRow(type, {
              status: "done",
              verification: verification === "timeout" ? null : verification,
              pending: false,
            });
            if (verification !== "timeout") router.refresh();
          }
          if (type === "freeze_sms") {
            try {
        ...
        {coreDocs.map((doc) => {
          ...
          return (
            <div key={doc.type} className="u-card px-4 py-3.5">
              ...
              {row.status === "done" && row.pending && aiOcrEnabled ? (
                <div
                ...
                >
                  ...
                  <p className="mt-1 text-[0.71875rem] text-[var(--ink-muted)]">
                    {t("aiCheckingSub")}
                  </p>
                </div>
              ) : null}
              ...
              {row.status === "done" ? (
                ...
                >
                  ...
                  row.verification?.documentKind === "pdf_no_text" ? (
                    ...
                    // Null verification = sealed only (timeout / opt-out / still pending).
