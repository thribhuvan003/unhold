...
const navLinks = [
  {
    ...
    emphasis: true,
    match: (path: string) => path.startsWith('/guest') || path.startsWith('/start'),
  },
  {
    href: '/cases',
    key: 'cases',
      ...
      <div className="mx-auto flex h-14 max-w-5xl items-center justify-between gap-3 px-4 sm:gap-4 sm:px-5">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <Link
            ...
            className="inline-flex min-h-[32px] items-center rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-[0.8125rem] font-bold text-white no-underline"
          >
            {t('languageName')}
          </Link>
          ...
          <a
            href="tel:1930"
            className="inline-flex items-center gap-1.5 rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-[0.8125rem] font-bold text-white no-underline sm:hidden"
          ...
          <nav aria-label="Main" className="u-nav-bar max-sm:hidden">
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  <span className="hidden min-[380px]:inline">{t(`nav.${link.key}`)}</span>
                  <span className="min-[380px]:hidden">{t(`nav.${link.shortKey}`)}</span>
                </Link>
              );
            })}
          </nav>
