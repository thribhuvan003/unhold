...
export function LetterWorkspace({
  ...
  // Bundle seal only clears evidence_bundle — prior-level proof stays blocked.
  const priorOrBundleBlocked =
    bundleGateBlocked && !(needsSealedBundle && bundleSealedLocal);
  const markSentBlocked = !isApproved || priorOrBundleBlocked;
  const markSentBlockedReason = !isApproved
    ? t('reviewFirst')
    : priorOrBundleBlocked
      ? bundleGateReason
      : undefined;
  return (
    <>
      <LetterPreview
        ...
        gateBlocked={!isApproved && priorOrBundleBlocked}
