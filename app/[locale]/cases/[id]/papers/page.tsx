...
import { isLetterEvidenceReady } from '@/lib/intake/letter-ready';
...
async function loadPapersData(caseId: string, guestToken: string | undefined, locale: string): Promise<PapersData> {
  ...
  const [{ data: caseRow }, { data: evidenceRows }, { data: events }, { data: notice }, aiOcrEnabled] = await Promise.all([
  ...
  return {
    ...
    extraDocs,
    reasonLabel,
    letterUnlocked: isLetterEvidenceReady(freezeReason, (type) => !!readableByType[type]),
    aiOcrEnabled: Boolean(aiOcrEnabled),
...
export default async function PapersPage({ params }: PageProps) {
  ...
  if (!data.authorized) {
    return (
      <section className="u-case-shell mx-auto flex w-full flex-col gap-4">
  ...
  return (
    <section className="u-case-shell mx-auto flex w-full flex-col gap-3.5">
      <Link
        href={`/cases/${id}`}
      ...
      <PapersChecklist
        ...
        aiOcrEnabled={data.aiOcrEnabled}
      />
