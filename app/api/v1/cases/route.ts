 ...
 */
async function resolveBankId(admin: AdminClient, slug: string): Promise<string | null> {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!bankId) {
      throw new ApiError(400, 'validation_failed', 'No banks are configured yet — please try again shortly.');
    }
    ...
    if (!auth.userId && auth.guestSessionId) {
      const { generateRecoveryCode, hashRecoveryCode, RECOVERY_HASH_KEY, RECOVERY_SET_AT_KEY } =
        await import('@/lib/auth/recovery');
      recoveryCode = generateRecoveryCode();
      const prevMeta =
        typeof created.metadata_json === 'object' && created.metadata_json !== null
