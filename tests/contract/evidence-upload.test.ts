...
vi.mock('@/lib/state-machine/transitions', () => ({
  tryAutoTransitionOnEvidence: (...args: unknown[]) => tryTransitionMock(...args),
}));
...
describe('evidence upload contract', () => {
  beforeEach(() => {
    ...
    evidenceInsertMock.mockResolvedValue({
      data: { id: evidenceId, storage_path: buildEvidenceStoragePath(caseId, evidenceId, 'stmt.pdf') },
      error: null,
    });
  ...
  it('POST upload-url returns signed URL and storage path', async () => {
    const request = guestRequest(`http://localhost/api/v1/cases/${caseId}/evidence/upload-url`, {
      method: 'POST',
    ...
    const response = await uploadUrl(request, { params: Promise.resolve({ id: caseId }) });
