...
describe('PapersChecklist', () => {
  ...
  it('rejects an irrelevant file even when the model over-reports confidence (client mirrors server cap)', async () => {
    ...
    await act(async () => {
      // Poll cadence is 2s; advance past the first poll tick.
  ...
  it('runs the real upload flow, polls the verifier, and never shows raw flag codes', async () => {
    ...
    await uploadTo('Add photo — Freeze SMS or notice');
    // Confirm returns immediately; AI runs in background (pending seal).
  ...
  it('falls back calmly when polling exhausts without a verifier event', async () => {
    ...
    await uploadTo('Add photo — Bank statement');
    // Saved immediately; background poll may still be pending.
    expect(container.textContent).toContain('Saved');

    // POLL_INTERVAL_MS=2000 × POLL_MAX_ATTEMPTS=30 → exhaust with empty events.
    await act(async () => {
      await vi.advanceTimersByTimeAsync(30 * 2000 + 500);
    });

    // Timeout clears spinner; sealed-only copy (not a fake 0% AI pass).
    expect(container.textContent).toContain(
      'review the original carefully before you use it',
    );
    expect(container.textContent).not.toContain('0% confidence');
    expect(container.textContent).toContain('1 of 3 added');
  });
