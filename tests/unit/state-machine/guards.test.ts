...
describe('state machine guards', () => {
  ...
  describe('evaluateTransition', () => {
    ...
    it('allows mark_sent from prep statuses when L1 is approved (letter product path)', () => {
      const statuses = ['new', 'intake_scoping', 'monitoring', 'evidence_building'] as const;
      const approved = [
        { id: 'l1', level: 'L1' as const, status: 'approved' as const, approved_at: 't', sent_proof_evidence_id: null },
      ];
      for (const status of statuses) {
        const result = evaluateTransition(
          ctx({ case: baseCase(status), escalations: approved }),
          'user.mark_sent',
        );
        expect(result.ok, status).toBe(true);
      }
    });
    it('still blocks mark_sent from prep statuses without an approved escalation', () => {
