...
describe('runEvidenceBundle', () => {
  ...
  it('merges a real PNG image evidence item as an A4 page', async () => {
  ...
  it('places a large phone-photo JPEG on an A4 page instead of a native-resolution page', async () => {
    const largeJpeg = await sharp({
      create: { width: 3000, height: 2000, channels: 3, background: { r: 180, g: 180, b: 180 } },
    })
      .jpeg()
      .toBuffer();
    storageDownloadMock.mockResolvedValue(downloadResult(largeJpeg));
    const { runEvidenceBundle } = await importRunner();

    const result = await runEvidenceBundle({
      case_id: 'case-10',
      case_public_id: 'LL-10',
      evidence: [
        {
          id: 'ev-1',
          storage_path: 'case-10/ev-1/photo.jpg',
          mime_type: 'image/jpeg',
          evidence_type: 'freeze_notice',
          created_at: '2026-01-04T00:00:00.000Z',
          sha256: computeSha256Hex(largeJpeg),
        },
      ],
    });
  ...
  it('keeps the ORIGINAL file sha256 in the manifest, not the hash of the resized presentation copy', async () => {
    const largeJpeg = await sharp({
      create: { width: 3000, height: 2000, channels: 3, background: { r: 180, g: 180, b: 180 } },
    })
      .jpeg()
      .toBuffer();
    const originalSha256 = computeSha256Hex(largeJpeg);
    const resized = await prepareForBundle(largeJpeg, 'image/jpeg');
    const resizedSha256 = computeSha256Hex(resized.buffer);
    storageDownloadMock.mockResolvedValue(downloadResult(largeJpeg));
    const { runEvidenceBundle } = await importRunner();

    const result = await runEvidenceBundle({
      case_id: 'case-11',
      case_public_id: 'LL-11',
      evidence: [
        {
          id: 'ev-1',
          storage_path: 'case-11/ev-1/photo.jpg',
          mime_type: 'image/jpeg',
          evidence_type: 'freeze_notice',
          created_at: '2026-01-04T00:00:00.000Z',
    ...
    expect(result.output.manifest).toHaveLength(1);
    expect(result.output.manifest[0].sha256).toBe(originalSha256);
    expect(result.output.manifest[0].sha256).not.toBe(resizedSha256);
  });
