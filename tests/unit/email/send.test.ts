...
function mockAdmin(opts?: {
  ...
  const rpc = vi.fn((fn: string) => {
    if (fn === 'claim_reminder_send') {
      return Promise.resolve({ data: opts?.claim ?? true, error: opts?.claimError ?? null });
    }
...
describe('sendDeadlineReminderForCase (opt-in gate + atomic claim)', () => {
  ...
  it('skips (no claim, no send) when the user has NOT opted in', async () => {
    ...
    const result = await sendDeadlineReminderForCase({
      ...baseCase,
      intake_json: { reminder_email: 'user@example.com', reminder_opt_in: false },
    });
  ...
  it('skips when the deadline has not lapsed yet', async () => {
    ...
    const result = await sendDeadlineReminderForCase({ ...baseCase, next_user_action_due_at: FUTURE });
  ...
  it('claim WINS once: sends exactly one reminder and records a swarm event', async () => {
    ...
    expect(rpc).toHaveBeenCalledWith('claim_reminder_send', { p_case_id: 'case-1', p_due: PAST });
    expect(fetchMock).toHaveBeenCalledOnce();
    // Won the claim → never releases it.
    expect(rpc).not.toHaveBeenCalledWith('release_reminder_send', expect.anything());
  ...
  it('RELEASES the claim when the send fails, so a later run retries', async () => {
    ...
    expect(result).toEqual({ outcome: 'skipped', reason: 'channel_send_failed' });
    expect(rpc).toHaveBeenCalledWith('claim_reminder_send', { p_case_id: 'case-1', p_due: PAST });
    expect(rpc).toHaveBeenCalledWith('release_reminder_send', { p_case_id: 'case-1', p_due: PAST });
    expect(fetchMock).not.toHaveBeenCalled();
    expect(appendSwarmEvent).not.toHaveBeenCalled();
  });
});
