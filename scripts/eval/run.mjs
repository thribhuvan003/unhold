...
async function judge(notice, analysis) {
  ...
  const user = JSON.stringify({ notice, analysis: { freeze_reason: analysis.freeze_reason, plain_english: analysis.plain_english, what_this_means: analysis.what_this_means, suggested_next: analysis.suggested_next } });
...
for (const c of data.cases) {
  ...
  if (!analysis) { rows.push({ id: c.id, classOk: false, fr: 'NULL', faith: 0, restriction: false, authority: false, halluc: true, ms }); continue; }
  ...
  console.log(`${c.id}: class=${classOk ? 'OK ' : 'MISS'} fr=${analysis.freeze_reason} faith=${verdict.faithfulness} restriction=${verdict.restriction_detail_grounded} authority=${verdict.authority_or_reference_grounded} (${(ms / 1000).toFixed(1)}s)`);
}
...
console.log(`restriction grounding:   ${(restrictionRate * 100).toFixed(0)}%  (all cases)`);
console.log(`authority/ref grounding: ${(authorityRate * 100).toFixed(0)}%  (all cases)`);
