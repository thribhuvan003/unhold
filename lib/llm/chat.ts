...
async function tryProvider(provider: Provider, options: ChatOptions): Promise<string | null> {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      // Bound provider wait so a hung vision call cannot pin a serverless
      // invocation until maxDuration (user already got HTTP via after()).
      const controller = new AbortController();
      const timeoutMs = options.vision ? 18_000 : 25_000;
