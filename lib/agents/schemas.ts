...
export const VerifierResultOutputSchema = z.object({
  ...
  human_review_required: z.boolean(),
  /** Is this a genuine freeze-evidence document at all (vs a blank/unrelated
   * file)? Defaults true for backward-compat with older stored outputs. */
  relevant: z.boolean().default(true),
  /** What the document looks like, for a tailored "wrong document" message. */
  document_kind: z.string().max(40).optional(),
});
