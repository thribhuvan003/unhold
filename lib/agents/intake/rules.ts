...
/**
 * True when the case text actually mentions a cyber/NCRP/UPI signal. Without
 * one, `cyber_upi_chain` is only a default (the DB enum has no "unknown") and
 * must be treated as low-confidence, never as a silent classification.
 */
function hasCyberSignal(input: IntakeClassifierInput): boolean {
  const blob = textBlob(input);
  const hint = String(input.intake_json.freeze_reason_hint ?? '').toLowerCase();
  if (input.intake_json.ncrp_id) return true;
  if (hint.includes('cyber') || hint.includes('upi')) return true;
  return (
    blob.includes('cyber') ||
    blob.includes('ncrp') ||
    blob.includes('upi') ||
...
function playbookFor(
  ...
  // victim_upi_chain_sbi is the only seeded victim playbook (see 010_rls_views_seed.sql).
  if (role === 'victim') return 'victim_upi_chain_sbi';
...
export function classifyIntakeFromRules(input: IntakeClassifierInput): IntakeClassificationOutput {
  ...
  // No silent bank default: an unknown bank must route to the generic playbook,
  // never to an SBI-specific one.
  const bankSlug = String(input.intake_json.bank_slug ?? '');
  ...
  // Defaulted cyber classification without any cyber signal in the case text:
  // keep the enum value but drop confidence so human review is required and
  // downstream copy says "needs bank clarification" instead of asserting cyber.
  if (freeze_reason === 'cyber_upi_chain' && !hasCyberSignal(input)) {
    confidence = 0.6;
    confidence_breakdown.rules = 0.6;
    confidence_breakdown.default_reason_no_signal = 0.6;
  }
