...
/**
 * Classify a case's freeze_reason NOW, inline. At case creation there is no
 * evidence yet, so this uses the deterministic rules classifier (no LLM, no
 * consent, instant) — which is exactly what we want so the papers checklist and
 * the letter can adapt to the freeze reason from the first screen. Previously
 * intake only ran when a notice was analysed, so freeze_reason stayed null and
 * every case got the generic 3-paper checklist.
 */
export async function classifyCaseNow(caseId: string): Promise<void> {
  const input = await loadIntakeClassifierInput(caseId);
  const output = await runIntakeClassification(input);
  await applyIntakeSideEffects(caseId, output, `inline:${caseId}`);
}