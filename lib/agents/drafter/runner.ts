...
import {
  ...
  type TemplateLevel,
} from "@/lib/agents/fallback/index";
import {
  chatCompletion,
  extractJsonText,
  isLlmConfigured,
...
const LEVEL_CHANNEL: Record<
  TemplateLevel,
  Database["public"]["Enums"]["escalation_channel"]
> = {
  L1: "branch_manager",
  L2: "nodal_officer",
  L3: "rbi_cms",
...
const TRACK_LETTER_COPY: Record<UnfreezeTrack, TrackLetterCopy> = {
  cyber: {
    ...
    situationFallback:
      "I am the account holder and need the written restriction details so I can respond through the correct official channel.",
    l1KeyRequests: (disputed, amountInr) =>
      `2. Please confirm whether the amount currently held is ${
        disputed
          ? `approximately Rs. ${amountInr}`
          : "the amount shown in the freeze record"
  ...
  court: {
    ...
    declaration:
      "I confirm the details above are true and correct to the best of my knowledge.",
...
export function buildCaseAwareLines(
  ...
  const reference = String(ncrpId ?? intake.ncrp_id ?? "").trim();
  ...
  return {
    FREEZE_DESCRIPTION,
    SITUATION_LINE,
    NCRP_REFERENCE_LINE: reference ? ` (NCRP reference ${reference})` : "",
...
export async function checkProofGate(
  ...
  if (!data) return true;
  if (data.status !== "sent" && data.status !== "response_received")
    return true;
...
export async function loadDrafterContext(
  ...
  const { data: caseRow, error } = await supabase
    .from("cases")
    .select(
      "id, intake_json, account_last4, ncrp_id, frozen_amount_paise, frozen_since, freeze_reason",
    )
    .eq("id", caseId)
    .single();
...
async function draftWithLlm(
  ...
  const text = await chatCompletion({
    ...
    messages: [
      {
        role: "system",
        content: buildDrafterSystemPrompt(ctx.level) + grounding,
      },
      {
        role: "user",
...
export async function runDrafterJob(job: AgentJobRow): Promise<AgentRunResult> {
  ...
  const level = String(
    payload.level ?? payload.target_level ?? "L1",
  ) as TemplateLevel;
