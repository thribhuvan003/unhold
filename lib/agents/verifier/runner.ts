...
/**
 * Formats we can run through vision OCR. HEIC/HEIF/WebP are accepted on upload
 * and normalised to JPEG via sharp in downscaleForVision before the model call.
 * application/pdf is handled separately via text extraction (not vision).
 */
...
async function extractWithLlm(
  ...
  if (input.mime_type === 'application/pdf') {
    ...
    // Downscale + JPEG-normalise before vision OCR (HEIC/WebP included).
    const { buffer, mime } = await downscaleForVision(original, input.mime_type ?? 'image/jpeg');
    // If sharp could not convert HEIC/etc., skip vision rather than send a
    // provider-rejected mime (null -> pending_human / queue retry path).
  ...
  const text = await chatCompletion({
    ...
    // Structured OCR JSON is small; lower cap trims TTFT on vision models.
    max_tokens: 1024,
    temperature: 0.1,
