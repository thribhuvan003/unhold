...
/**
 * Hard time budget for RAG grounding (embed → pgvector → rerank, three network
 * calls). Grounding is best-effort — measured at 1.9–3.6s when the embedding /
 * rerank endpoints are slow, it was the dominant cost of the whole analysis.
 * Past this budget we proceed ungrounded rather than keep the user waiting.
 */
const GROUNDING_BUDGET_MS = 1500;

function withTimeout<T>(promise: Promise<T>, ms: number): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('grounding_timeout')), ms);
    promise.then(
      (value) => {
        clearTimeout(timer);
        resolve(value);
...
export async function analyzeNotice(input: NoticeAnalyzerInput): Promise<NoticeAnalysisOutput | null> {
  ...
  // notice text (pasted or PDF-extracted). Best-effort and time-boxed: on budget
  // overrun we analyze ungrounded (empty grounding is a first-class prompt path).
  let grounding = '';
  if (groundingQuery) {
    try {
      const chunks = await withTimeout(retrieveRelevantContext(groundingQuery, 5), GROUNDING_BUDGET_MS);
      grounding = chunks
        .map(
          (c) =>
