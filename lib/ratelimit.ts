...
function getSwarmEventsReadLimiter(): Ratelimit | null {
  ...
  return new Ratelimit({
    redis,
    // Normal UI polls ~every 2s during verify/draft (~150/5min max). Keep headroom.
    limiter: Ratelimit.slidingWindow(180, "5 m"),
  ...
  if (limiter) {
    ...
    if (!success) {
      throw new ApiError(
        429,
        "rate_limited",
        "Swarm events read limit exceeded (180/5min/case)",
      );
    }
    return;
  }
