...
export async function enforceCaseRecoverLimit(identifier: string): Promise<void> {
  const limiter = getCaseRecoverLimiter();
  if (limiter) {
    const { success } = await limiter.limit(identifier);
    if (!success) {
      throw new ApiError(429, 'rate_limited', 'Too many recovery attempts. Try again later.');
    }
    return;
  }

  const allowed = await memoryRateLimit(`case_recover:${identifier}`, 5, 60 * 60 * 1000);
  if (!allowed) {
    throw new ApiError(429, 'rate_limited', 'Too many recovery attempts. Try again later.');
  }
}
