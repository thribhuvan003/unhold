...
/** Hang protection: a stalled rerank call must not block the caller — on
 * timeout we return null and the caller keeps the vector order. */
...
export async function rerankPassages(
  ...
  for (let attempt = 0; attempt < Math.min(keys.length, 3); attempt++) {
    try {
      ...
      // Timeout means the endpoint is slow/hung — bail; callers fall back to
      // the vector order rather than waiting on another timed-out attempt.
