...
**/healthz note:** next-intl middleware previously treated /healthz as a locale page and returned a soft HTML 404. proxy.ts now rewrites /healthz and /health to /api/v1/health before locale routing. Prefer monitoring https://www.unhold.live/healthz or /api/v1/health (JSON { ok: true }).
